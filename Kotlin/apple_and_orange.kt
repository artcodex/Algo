...
fun countApplesAndOranges(s: Int, t: Int, a: Int, b: Int, apples: Array<Int>, oranges: Array<Int>): Unit {
    ...
    val appleDistances = apples.map { it + a }
    val orangeDistances = oranges.map { it + b }
    
    val appleCount = appleDistances.fold(0) { acc, v -> 
        if (v in s..t) 
            acc + 1
        else 
            acc
    ...
    val orangeCount = orangeDistances.fold(0) { acc, v -> 
        if (v in s..t) 
            acc + 1
        else 
            acc
